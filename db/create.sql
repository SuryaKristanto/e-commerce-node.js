...
CREATE TABLE IF NOT EXISTS orders(id INT PRIMARY KEY AUTO_INCREMENT NOT NULL, user_id INT NOT NULL, order_no INT NOT NULL UNIQUE, status VARCHAR(255) DEFAULT 'WAITING FOR PAYMENT' NOT NULL, payment_method VARCHAR(255) NOT NULL, total_price INT, updated_at TIMESTAMP NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, deleted_at TIMESTAMP NULL DEFAULT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS wishlist(id INT PRIMARY KEY AUTO_INCREMENT NOT NULL, user_id INT NOT NULL, product_code INT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE, FOREIGN KEY (product_code) REFERENCES products(code) ON UPDATE CASCADE ON DELETE CASCADE);
